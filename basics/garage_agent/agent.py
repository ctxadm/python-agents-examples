...
from livekit.agents import JobContext, WorkerOptions, cli, RunContext, AgentSession, Agent
...
class GarageAssistant(Agent):
    ...
    def __init__(self) -> None:
        # Instructions ANGEPASST für Autowerkstatt Zürich
        super().__init__(instructions="""Du bist Pia, der digitale Assistent der Autowerkstatt Zürich.
...
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        # Verwende with_ollama für Llama 3.2
        ...
        # 8. KRITISCH: Begrüßung NACH session.start() mit längerem Delay
        await asyncio.sleep(2.0)  # Längerer Delay für Session-Initialisierung
        
        logger.info(f"📢 [{session_id}] Sending initial greeting...")
        
        try:
            # generate_reply ohne tool_choice für erste Begrüßung
            await session.generate_reply(
                instructions="""Begrüße den Kunden freundlich als Pia von der Autowerkstatt Zürich.
                Sage: 'Guten Tag und willkommen bei der Autowerkstatt Zürich! Ich bin Pia, Ihre digitale Assistentin. Wie kann ich Ihnen heute helfen?'
                NICHTS ANDERES! Keine weiteren Erklärungen!"""
            )
        ...
        try:
            ...
            logger.error(f"[{session_id}] Fallback greeting also failed: {e2}")
